...
model: `docs/SECURITY.md`. Auth/session sections (V2/V3) refreshed against
real authentication (AGENTS.md §3ff/§3hh) — see those sections' notes for
what changed from the original Tier-2/no-real-auth baseline.
...
| # | Control | ASVS | Status | Phase | Notes |
|---|---|---|---|---|---|
| 6 | Password hashing: Argon2id (bcrypt fallback only) | V2.4 (L1/L2) | ✅ | ad hoc (§3ff) | `src/server/auth/credentials.ts`'s `verifyCredentials` — `argon2` package, Argon2id (the library's default mode). No bcrypt fallback exists or is needed; every real password in this app was hashed with Argon2id from the start, so there's no legacy-bcrypt population to migrate |
| 7 | WebAuthn/passkeys as primary factor | V2.1, V2.2 (L2) | ⬜ | deferred | Explicit scope decision, not an oversight (AGENTS.md §3ff's opening framing): this app has no registered OAuth app and no outbound email infrastructure, and Credentials (email + Argon2id) was the only strategy that could be built *and actually verified end to end* in this environment. Revisit if a real OAuth provider or WebAuthn relying-party setup is ever available to build against |
| 8 | TOTP second factor + hashed recovery codes | V2.5 (L2) | 🟡 | ad hoc (§3hh) | TOTP itself is real and live-verified: `src/server/auth/totp.ts` (otplib v13, RFC 6238, ±1 step clock-drift tolerance), `User.totpSecret` encrypted at rest via the same AES-256-GCM codec as item 32, `totpLastUsedTimeStep` giving real replay protection (the identical 30-second code can't be accepted twice), enforced inside `authorize()` after password verification (`checkTotpChallenge`). **Still 🟡, not ✅**: no hashed recovery codes exist — losing the authenticator app before disabling MFA locks the account out with no self-service recovery, an explicitly documented known limitation (AGENTS.md §3hh), not a gap that was missed |
| 8a | Password reset: time-limited, single-use, cryptographically random tokens; TOTP required to complete reset if enabled | V2.5, V3.4 (L2) | ✅ | ad hoc (§3jj) | `PasswordResetToken` (SHA-256 `tokenHash` only, never the raw token, at rest), 15-minute expiry, single-use, invalidated on a fresh request. Reusing `totp.ts`'s real verification (not a KBA/security-question fallback — this app never had one). Uniform "if an account exists…" response on request, regardless of outcome, including a Resend outage (`requestPasswordReset` catches/logs its own send failure rather than throwing) |
| 8b | Email address verification | V2.10 (custom) | 🟡 | ad hoc (§3jj) | `EmailVerificationToken`, same shape as 8a, 24h expiry. Sent automatically at registration and resendable from `/settings`. **Still 🟡**: verification status is informational only today — no feature is gated on it (e.g. password-reset/login don't currently require a verified email), a deliberate scope decision matching this pass's own "don't silently expand scope" discipline, not an oversight |
| 8c | Login rate limiting / anti-automation lockout | V2.2 (L1/L2) | ✅ | ad hoc (§3jj) | `credentials.ts`'s `checkLoginRateLimit` — the same in-memory sliding-window limiter every other bootstrap route already uses (registration's per-email limit), 10 attempts/15 min keyed by submitted email, checked in `authorize()` BEFORE `verifyCredentials` so a locked-out account never pays the Argon2id hashing cost. Per-email, not per-IP, so it bounds credential stuffing regardless of source IP rotation |
| 9 | No user enumeration (identical timing/messaging) | V2.1, V2.6 (L2) | ✅ | ad hoc (§3ff) | `verifyCredentials` returns `null` uniformly for all three failure shapes (unknown email, an unclaimed seeded row, a wrong password) — a login form can't distinguish which one occurred. `argon2.verify` is constant-time by construction (comparing against a real Argon2id hash, not a plaintext), so no separate timing-safe-compare step was needed the way item 10 needed one for Origin/Host. TOTP's "required" vs. "invalid" responses (item 8) are deliberately NOT uniform, but only ever reached after the password was already proven correct, so they don't reveal account existence pre-password |
| 10 | Constant-time comparisons (`crypto.timingSafeEqual`) for tokens/secrets | V2.1 (L1/L2) | ✅ | ad hoc (§3ff) | Now has a real target: `argon2.verify`'s own constant-time comparison (item 9's note) is what actually protects the one real secret-comparison this app performs at login. `src/server/api/verify-origin.ts`'s Origin/Host match still uses `timingSafeEqual` too, still defense-in-depth/ASVS-habit rather than closing a real timing side-channel (Origin/Host are public, client-sent values, not secrets) |
| 11 | Real multi-user authentication, server-resolved, never client-supplied | V2.1 | ✅ | ad hoc (§3ff) | `src/server/auth/current-user.ts`'s `getCurrentUser()` contract is unchanged (still always resolves a real `User` row or throws) — its *internals* now read a real Auth.js JWT session instead of a hardcoded seeded email. The first real registration inherits the original seeded demo user's data (`registerUser`'s `inherited` flag, integration-tested to never touch the two other unclaimed seeded rows — the household members from §3s); every subsequent registration is a genuinely fresh, isolated account. `src/proxy.ts` is the actual authentication gate (not `getCurrentUser()` — see item 12), default-protected with an explicit allowlist for `/login`, `/register`, `/welcome`, `/api/auth/**`, and the two Dead Man's Switch beneficiary-recovery routes (§3t), since a beneficiary is by design not this app's own authenticated user |

## V3 — Session Management

| # | Control | ASVS | Status | Phase | Notes |
|---|---|---|---|---|---|
| 12 | Server-managed sessions, no unrevokable stateless JWTs | V3.2 (L1/L2) | ✅ | ad hoc (§3hh) | Auth.js JWT-strategy sessions (`session: { strategy: "jwt" }`, `src/server/auth/auth.ts`) — a deliberate departure from the literal "server-managed sessions" wording (no `Session` table; see AGENTS.md §5 decision #1 and §3ff for why: no verified-compatible Prisma adapter for this app's Prisma 7 TS-source generator mode), but the control's actual intent — a JWT that CAN be revoked server-side — is real: `User.tokenVersion` (default 1) is re-checked on every non-sign-in request inside the `jwt()` callback against the value baked into the token at sign-in; a mismatch returns `null`, which — verified directly against the installed `@auth/core` source, not assumed from docs — clears the session cookie rather than re-signing it. Two real triggers: `POST /api/auth/revoke-sessions` ("Sign out of all sessions" in `/settings`) and disabling TOTP MFA, both live-verified across two genuinely separate concurrent sessions |
| 13 | `__Host-` prefixed, `HttpOnly`, `Secure`, `SameSite=Strict`, short TTL | V3.4 (L1/L2) | ✅ | ad hoc (§3ff → §3jj) | **Closed in §3jj.** `auth.ts` now overrides `cookies.sessionToken` — `__Host-authjs.session-token` (no `Domain` attribute, `Path=/`), `SameSite=Strict`, `Secure: true` — and `session.maxAge` is 7 days (from Auth.js's 30-day default), `updateAge` 1 day. **Deliberately gated on `APP_URL` actually being `https://`, not `NODE_ENV === "production"`** — a real bug caught while building the e2e login flow: `next start` sets `NODE_ENV=production` internally even over plain HTTP (e.g. this app's own Playwright suite, `http://localhost:3100`), so gating on `NODE_ENV` alone would have applied `Secure`/`__Host-` to a cookie that was never actually HTTPS, breaking sign-in for any plain-HTTP `next start` deployment. `getAppUrl()`'s protocol mirrors Auth.js's OWN `useSecureCookies = url.protocol === "https:"` default (verified against the installed `@auth/core` source) using a value this static config CAN see, so local dev/e2e keep Auth.js's ordinary cookie behavior unchanged. `SameSite=Strict`'s accepted trade-off: a link to this app clicked FROM an external site won't carry the session cookie on that first cross-site navigation — acceptable since no emailed link (password-reset/email-verification included) ever points at a page requiring an existing session |
| 14 | Session ID rotation on privilege change | V3.2 (L2) | 🟡 | ad hoc (§3hh) | No literal session-ID rotation exists (JWT-strategy sessions have no server-side session ID to rotate) — but the control's real intent, invalidating a session across a genuine privilege/security-posture change, is achieved differently: `bumpTokenVersion()` invalidates *every* outstanding session (including the caller's own) rather than rotating just one, fired on disabling TOTP MFA (a real security downgrade) and on-demand via "Sign out of all sessions." Coarser than per-session rotation, not finer — a deliberate, documented trade-off, not an oversight |
...
| 18 | Negative IDOR tests: User B → User A's resource returns 404 (never 403) | V4.1, V4.3 (L1/L2) | 🟡 | 2→4→7 | `tests/integration/idor.test.ts` proves it at the DAL layer across 4 models. **Phase 7**: `tests/e2e/security.spec.ts` automates the HTTP-level check too — a nonexistent transaction ID against the live route returns a real `404 {"error":"Not found"}` with no stack trace, and a positive control on the same route proves the check isn't just failing everything closed. Still 🟡, updated (not closed) now that real auth exists (§3ff): the DAL/RLS layer's own cross-user isolation is proven repeatedly by feature-specific integration suites with two real distinct users (e.g. `tests/integration/shared-groups.test.ts`, §3s; `tests/integration/crypto-wallets.test.ts`, §3w), but a dedicated HTTP-level test using two real, separately-authenticated Auth.js sessions (real cookies, not the admin client) hitting the SAME generic mutating route hasn't been written yet — a genuinely buildable regression test now that login exists, just not built in this pass |
...
- `tests/guards/admin-client-boundary.test.ts` — only the seed script and a small, explicitly allowlisted set of bootstrap-problem callers may use the RLS-bypassing admin client: `current-user.ts` (Phase 4), the household/vault invite-accept flows (`invite-admin-ops.ts`/`recovery-admin-ops.ts`, §3s/§3t), the Dead Man's Switch batch inactivity check (§3t), and `credentials.ts` (login/registration, §3ff) — each shares the identical justification: resolving or creating a `userId` has to happen before any row-level RLS standing exists to scope a normal `withUserScope` call by
- `tests/guards/zk-client-only.test.ts` — the zero-knowledge goal-notes vault's crypto (§3m) may never be imported from `src/server/**`
- `tests/guards/dead-mans-switch-crypto-client-only.test.ts` — same pattern, for the Emergency Vault's crypto (§3t)
- `tests/guards/receipt-ocr-client-only.test.ts` — same pattern, for Tesseract.js OCR (§3q)
- `tests/guards/local-embedder-client-only.test.ts` — same pattern, for the client-side embedding model (§3u/§3bb)
- `tests/guards/forecaster-client-only.test.ts` — same pattern, for the ONNX cash-flow forecaster (§3dd)
...
As of this check (§3gg/§3hh production-verification pass): **8 advisories (2 moderate, 6 high)**, all transitive, all in unreachable code paths — none newly introduced by application code, none shipped in the client bundle or the production server's real request path:

- **2 moderate `qs`** (`GHSA-q8mj-m7cp-5q26`, a `qs.stringify` DoS on malformed input), transitive through `@stryker-mutator/core` → `typed-rest-client` → `qs`. Accepted as-is: no compatible fix upstream (would need a breaking `typed-rest-client` major with no confirmed-working replacement), and Stryker is a `devDependency` invoked only by `npm run test:mutation`, never bundled into the app or `next build`'s output.
- **4 high, `@huggingface/transformers`'s optional Node backend** (`onnxruntime-node` → `adm-zip`; `sharp`, no fix available) — documented since §3u: this app's browser build uses the WASM backend exclusively (enforced by `tests/guards/local-embedder-client-only.test.ts` keeping the package out of `src/server/**`), so `onnxruntime-node`/`sharp` are never the code path a real request executes. The one exception, `scripts/backfill-embeddings.ts` (§3ee), deliberately DOES run the Node backend — but as an operator-run maintenance script against trusted local input, the same risk class as `prisma db seed`, not a live-request attack surface.
- **2 high, `prisma`'s `mysql2` driver dependency** (`GHSA-3f6p-5ww8-9rcr`, an auth-plugin-downgrade credential leak) — **newly found and documented in this pass, not previously recorded**. `npm audit fix --force`'s suggested fix is `prisma@6.19.3`, an actual *downgrade* from this app's installed `7.10.0` (no compatible 7.x fix exists yet upstream), so it isn't applied. Confirmed unreachable, not just assumed: this app's `datasource` provider is `postgresql` exclusively (`prisma/schema.prisma`), `src/server/db/client.ts`/`admin-client.ts` construct `PrismaClient` with `@prisma/adapter-pg` only, and a repo-wide grep for `mysql` outside test fixtures/node_modules returns nothing — the Prisma CLI simply bundles driver support for every database it can target, and this app never configures or loads the MySQL one.

Re-check whenever `@stryker-mutator/core`, `@huggingface/transformers`, or `prisma` are next bumped — each of these three clusters is independently worth re-testing for a real fix landing upstream.
