...
| 13 | `__Host-` prefixed, `HttpOnly`, `Secure`, `SameSite=Strict`, short TTL | V3.4 (L1/L2) | 🟡 | ad hoc (§3ff) | Auth.js's untouched defaults — no `cookies` override exists in `auth.ts`. `HttpOnly` ✅ and `Secure` ✅ (auto-enabled in production via `useSecureCookies`) are real; three gaps remain, none yet closed: the cookie name is `__Secure-authjs.session-token` in production, not `__Host-`-prefixed (which additionally requires no `Domain` attribute — not currently configured either way); `SameSite` defaults to `Lax`, not `Strict`; and the JWT's `maxAge` is Auth.js's default 30 days, not a short TTL — item 12's `tokenVersion` revocation is what actually bounds a compromised token's real-world lifetime today, not session expiry. Revisit if this ever needs to be closed for a real (non-Tier-2-demo) deployment |
