    ...
    <div className="flex flex-col gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          {SUGGESTED_PROMPTS.map((prompt) => (
            <button
              ...
              className="uv-btn-press rounded-full border border-border px-3 py-1.5 text-xs font-medium text-muted transition-colors hover:bg-elevated-hover focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
            >
              {prompt}
            </button>
          ))}
        ...
        {messages.map((message, index) => (
          <div
            ...
            className={`max-w-[85%] whitespace-pre-wrap rounded-lg px-4 py-3 text-sm ${
              message.role === "user" ? "self-end bg-accent text-bg" : "self-start border border-border bg-surface text-fg"
            }`}
          >
      ...
      <form onSubmit={handleSubmit} className="flex items-end gap-2">
        ...
        <textarea
          ...
          className="flex-1 resize-none rounded-md border border-border bg-elevated px-3 py-2 text-sm text-fg placeholder:text-muted focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
        />
        <button
          type="submit"
