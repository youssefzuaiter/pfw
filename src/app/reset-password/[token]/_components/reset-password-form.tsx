...
export function ResetPasswordForm({ token }: { token: string }) {
  ...
  if (done) {
    return <p className="mt-6 text-sm text-slate-100">Your password has been reset. Redirecting you to sign in…</p>;
  }

  if (totpRequired) {
    return (
      <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-4">
        <p className="text-sm text-slate-400">This account has two-factor authentication enabled — enter the 6-digit code from your authenticator app to finish resetting your password.</p>
        <div className="flex flex-col gap-1">
          <label htmlFor="reset-password-totp-code" className="text-xs font-medium text-slate-400">
            Authentication code
          </label>
          <input
            ...
            className="rounded-md border border-slate-800/80 bg-slate-900 px-3 py-2 text-sm text-slate-100 font-tabular-figures tracking-tight placeholder:text-slate-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
          />
        </div>
        <button
          type="submit"
  ...
  return (
    <form onSubmit={handleSubmit} className="mt-6 flex flex-col gap-4">
      <div className="flex flex-col gap-1">
        <label htmlFor="reset-password-new" className="text-xs font-medium text-slate-400">
          New password
        </label>
        <input
          ...
          className="rounded-md border border-slate-800/80 bg-slate-900 px-3 py-2 text-sm text-slate-100 placeholder:text-slate-400 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring"
        />
