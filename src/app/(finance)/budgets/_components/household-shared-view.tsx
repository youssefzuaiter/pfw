  ...
  return (
    <div className="flex flex-col gap-4">
      <section className="rounded-lg border border-slate-800/80 bg-slate-900 p-4">
        <h2 className="mb-3 text-xs font-medium uppercase tracking-wide text-slate-400">Shared envelopes</h2>
        {envelopeAllocations.length === 0 ? (
          <p className="text-sm text-slate-400">No envelopes shared into this household yet.</p>
        ) : (
          <ul className="flex flex-col gap-2">
            {envelopeAllocations.map((allocation) => (
              <li key={allocation.id} className="flex flex-wrap items-center justify-between gap-2 border-t border-slate-800/80 pt-2 first:border-t-0 first:pt-0">
                <div>
                  <p className="text-sm text-slate-100">{allocation.category.name}</p>
                  <p className="text-xs text-slate-400">Shared by {ownerLabel(allocation.user)}</p>
                </div>
                <p className="font-tabular-figures text-sm tracking-tight text-slate-100">
      ...
      <section className="rounded-lg border border-slate-800/80 bg-slate-900 p-4">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          <h2 className="text-xs font-medium uppercase tracking-wide text-slate-400">Shared accounts</h2>
          {bankAccounts.some((a) => a.currency !== "ILS") && <CurrencyToggle />}
        </div>
        {bankAccounts.length === 0 ? (
          <p className="text-sm text-slate-400">No accounts shared into this household yet.</p>
        ) : (
          <ul className="flex flex-col gap-2">
            {bankAccounts.map((account) => (
              <li key={account.id} className="flex flex-wrap items-center justify-between gap-2 border-t border-slate-800/80 pt-2 first:border-t-0 first:pt-0">
                <div>
                  <p className="text-sm text-slate-100">{account.nickname ?? account.institutionName}</p>
                  <p className="text-xs text-slate-400">Shared by {ownerLabel(account.user)}</p>
                </div>
                <div className="text-right">
                  <CurrencyAmount
                    ...
                    primaryClassName="font-tabular-figures text-sm tracking-tight text-slate-100"
                    secondaryClassName="font-tabular-figures text-xs tracking-tight text-slate-400"
                  />
                </div>
              </li>
            ))}
      ...
      <section className="rounded-lg border border-slate-800/80 bg-slate-900 p-4">
        <h2 className="mb-3 text-xs font-medium uppercase tracking-wide text-slate-400">Shared categories</h2>
        {categories.length === 0 ? (
          <p className="text-sm text-slate-400">No categories shared into this household yet.</p>
        ) : (
